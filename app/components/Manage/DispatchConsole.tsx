...
import {
    Calendar, User as UserIcon, CheckCircle, AlertCircle,
    Clock, Truck, ChevronRight, UserCheck, UserX, BarChart3, CreditCard, ShieldCheck
} from "lucide-react";
...
export default function DispatchConsole() {
    ...
    const handleTabChange = (tab: "DISPATCH" | "PERSONNEL" | "SETTLEMENT") => {
        setActiveTab(tab);
        // We don't push router here to avoid full reloads, but could shallow push if needed.
    };
    ...
    const RenderDispatchTab = () => {
        ...
        return (
            <div className="flex h-full overflow-hidden bg-white rounded-xl shadow-sm border border-slate-200">
                ...
                {/* MIDDLE: Installers */}
                <div className="w-1/3 border-r bg-slate-50 flex flex-col min-w-[300px]">
    ...
    const RenderPersonnelTab = () => {
        ...
        return (
            <div className="flex flex-col md:flex-row h-full gap-6 overflow-hidden">
                {/* Pending List */}
                <div className="flex-1 flex flex-col min-w-[320px] max-w-md bg-white rounded-xl shadow-sm border border-slate-200">
                ...
                {/* Active List */}
                <div className="flex-[2] flex flex-col bg-white rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="h-full flex flex-col space-y-4">
            ...
            <div className="flex bg-slate-100/50 p-1 rounded-xl w-fit">
                <button onClick={() => handleTabChange("DISPATCH")}
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 ${activeTab === "DISPATCH" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>
                    <Truck size={14} />
                    배차 관제
                </button>
                <button onClick={() => handleTabChange("PERSONNEL")}
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 ${activeTab === "PERSONNEL" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>
                    <UserCheck size={14} />
                    시공/기사 관리
                    {installers.some(u => u.status === "PENDING") && <span className="inline-block w-2 h-2 bg-red-500 rounded-full ml-1"></span>}
                </button>
                <button onClick={() => handleTabChange("SETTLEMENT")}
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 ${activeTab === "SETTLEMENT" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}>
                    <CreditCard size={14} />
                    정산 관리
                </button>
            </div>
