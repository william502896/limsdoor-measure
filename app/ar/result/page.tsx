...
import {
    DoorStructure, FrameColor, GlassType, DesignType,
    DOOR_STRUCTURES, FRAME_COLORS, GLASS_TYPES, DOOR_DESIGNS
} from "../types"; // types now re-exports from catalog, but we need values too. 
// Actually types.ts only re-exports types. We should import values from catalog.
import {
...
export default function ArResultPage() {
    ...
    // Initial Load
    useEffect(() => {
        const raw = localStorage.getItem("limsdoor_ar_session_v1");
        if (raw) {
            try {
                const data = JSON.parse(raw);
                setImgUrl(data.imageDataUrl);
                setQuad(data.quad);
    ...
    const drawPreview = async () => {
        ...
        // 2. Draw Background (Using DetectFitMode)
        const imgW = baseImg.naturalWidth;
        const imgH = baseImg.naturalHeight;

        // Auto-detect fit mode (contain vs cover) based on container/image ratios
        ...
        // 3. Generate Door Asset
        const DOOR_TEX_W = 600;
        const DOOR_TEX_H = 1200;
        const doorDataUrl = await generateDoorPngDataUrl(config, DOOR_TEX_W, DOOR_TEX_H);

        const doorImg = new Image();
        doorImg.src = doorDataUrl;
        await new Promise(r => doorImg.onload = r);

        // 4. Warp using Robust Coords
        // Quad (0-1) -> Image Pixel -> Screen Pixel
        // Image Pixel = pt.x * imgW
        // Screen Pixel = drawX + (Image Pixel * scale)
        const p = quad.map(pt => ({
