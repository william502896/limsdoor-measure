        ...
        try {
            ...
            const { error: schedError } = await supabase
                ...
                .insert({
                    ...
                    title: `${selectedOrder.items[0]?.category || "시공"} 완료`,
                    type: "install",
                    status: "completed",
            ...
            if (schedError) {
                ...
                alert("서버 전송 중 오류가 발생했습니다. (로컬에는 저장됨)");
            } else {
                // Send Notification to Admin linked to ERP
                addNotification({
                    id: `notif-${Date.now()}`,
                    type: "INSTALL_COMPLETE",
                    message: `[시공 완료] ${user?.name || "시공기사"} 님이 ERP로 사진을 전송했습니다.`,
                    subText: `${selectedOrder.items[0]?.category} | ${selectedCustomer?.address || "주소 미상"}`,
                    timestamp: new Date().toISOString(),
                    isRead: false,
                    link: "/admin/apps/erp" // Go to ERP Dashboard
                });

                alert("시공 완료 및 ERP 전송 성공!");
            }
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex gap-2">
                                                    <input type="text" placeholder="서울시 강남구..." className="flex-1 text-base bg-white border border-slate-300 rounded-lg px-3 py-2.5 text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm" value={regForm.address} onChange={e => setRegForm({ ...regForm, address: e.target.value })} />
                                                    <button
