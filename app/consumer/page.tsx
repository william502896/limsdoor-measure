        ...
        async function fetchCompanyInfo() {
            try {
                ...
                const { data, error } = await supabase
                    .from('회사들')
                    .select('"카톡", "홈페이지", "쇼핑몰", "유튜브", "틱톡", "인스타그램", "페이스북", "portfolio_url", "google_photos_url"')
                    .eq('id', companyId)
                    .single();
                ...
                if (data) {
                    ...
                    setCompany({
                        ...
                        portfolio_url: companyData['portfolio_url'],
                        google_photos_url: companyData['google_photos_url']
                    });

                    // Fallback: If urls are missing in '회사들' (legacy schema), try fetching from 'company_settings' via API
                    if (!companyData['portfolio_url'] || !companyData['google_photos_url']) {
                        fetch('/api/company/settings')
                            ...
                            .then(json => {
                                setCompany(prev => {
                                    ...
                                    return {
                                        ...prev,
                                        portfolio_url: json.data?.portfolio_url || prev.portfolio_url,
                                        google_photos_url: json.data?.google_photos_url || prev.google_photos_url,
                                    };
                                });
                            })
                            .catch(err => console.error("Fallback settings fetch failed", err));
                    }
                }
            } catch (e) {
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 relative overflow-hidden" onClick={() => isLangOpen && setIsLangOpen(false)}>
            ...
            <div className="relative z-10 w-full max-w-md flex flex-col items-center text-center">
                <h1 className="text-5xl font-black tracking-tighter mb-2" style={{ fontFamily: 'var(--ui-font-heading)' }}>{PLATFORM_NAME}</h1>
                ...
                <div className="w-full space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Link href={company?.portfolio_url || "/shop/portfolio"} target={company?.portfolio_url ? "_blank" : undefined} className="p-4 bg-slate-800/80 backdrop-blur rounded-xl text-slate-300 font-bold hover:bg-slate-700 transition flex items-center justify-center">
                            {t.casesBtn}
                        </Link>
                        ...
                        {company?.google_photos_url && (
                            <Link href={company.google_photos_url} target="_blank" className="col-span-2 p-4 bg-blue-900/40 backdrop-blur rounded-xl text-blue-100 font-bold hover:bg-blue-800/60 transition flex items-center justify-center gap-2 border border-blue-500/30">
                                <span>📷 구글 사진첩</span>
                            </Link>
                        )}
                    </div>
